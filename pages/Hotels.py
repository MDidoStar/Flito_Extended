...
st.set_page_config(page_title="FLITO: Hotels", page_icon='logo.png', layout="wide")
...
# --- Page ---
st.title("🏨 Hotels")
st.write("Search nearest and best hotels now!")
...
if st.button('🔎 Search now', key='hotel_search'):
    if not hotel_city or not hotel_country:
        ...
        if prefs:
            ...
            """
        prompt = f"""
            Suggest for me a {val_rating}-stars hotel in {hotel_city}
            and in {hotel_country} with {hotel_price} price range.
